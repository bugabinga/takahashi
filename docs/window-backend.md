...
needs markup and audio — so text quality is first-class, which points to
...
build fetches nothing. The window backend links only system OpenGL/X11; text
links system FreeType/HarfBuzz. The SessionStart hook installs those `-dev`
packages via `apt`.
...
text (FreeType + HarfBuzz), images laid out in an equal grid (stb_image), and
...
`zig build test-media` (verified: text incl. Japanese, image, audio).
